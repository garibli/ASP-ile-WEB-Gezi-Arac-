...
public partial class SKSEkrani_IlanEkle : System.Web.UI.Page
{
    ...
    protected void btnGonder_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            altkontenjanBolumId = Convert.ToInt16(ddlIlgiliBolum.SelectedItem.Value);
        }
        catch
        {
            altkontenjanBolumId = -1;

        }
        int ustkontenjanBirimId = Convert.ToInt16(ddlIlgiliBirim.SelectedItem.Value);

        //kontenjan kontrolu
        if (altkontenjanBolumId != -1)
        {
            //verilen ilanlardak toplam kontenjan sayısı
            int bolumIdGetir = Convert.ToInt16(ddlIlgiliBolum.SelectedItem.Value);
            try
            {
                var ilanlıKontenjan = from kontenjanBak in turna.tblIlanlar where kontenjanBak.bolumID == bolumIdGetir select new { kontenjanBak.kontenjan };
            ...
            var bolumKontenjanSiniri = from bolum in turna.tblBolumler where bolum.id == bolumIdGetir select new { bolum.altKontenjan };
            foreach (var item in bolumKontenjanSiniri)
            {
                kontenjanBul += Convert.ToInt16(item.altKontenjan);
            }

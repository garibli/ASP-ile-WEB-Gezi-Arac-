...
public partial class userControl_Login : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Page.SetFocus(btnGonder);

        //if (Session["KisitliYetki"] == null || Session["KisitliYetki"] != "1")
        //{
            Response.Redirect("./KisitliYetki.aspx");
        //}
    }
    protected void btnGonder_Click(object sender, EventArgs e)
    {
        ...
        turna.PersonelGiris(txtMail.Text, txtSifre.Text, persMi);

        if ( Convert.ToInt16(persMi.Value) != 0)
        {
            
            var yetkiGetir = from calisan in turna.tblPersonel
                             where calisan.mail == txtMail.Text
        ...
        {
            turna.ogrGiris(txtMail.Text, txtSifre.Text, ogrMi);
            if (Convert.ToInt16(ogrMi.Value) != 0)
            {
                ...
                var ogr1 = from ogr in turna.tblOgrenci 
                           where ogr.mail == txtMail.Text
                           select new { ogr.ad, ogr.soyad, ogr.ogrNo};

                var ogrExt = from ogr in turna.tblOgrenci join calisMa in turna.tblCalismaYerleri 
                ...
                var anketT = from ogrAnket in turna.tblOgrenciAnketCevablari
                             where ogrAnket.ogrMail == txtMail.Text

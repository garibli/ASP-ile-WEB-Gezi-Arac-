...
public partial class ciktiIslemleri_BirimBordro : System.Web.UI.Page
{
    ...
    {
        if (Session["Mail"] == null || Session["bordroYil"] == null)
        ...
        dt.Columns.Add("netUcret");//20
        dt.Columns.Add("calisilanGunHesabi");//21
        dt.Columns.Add("ucretGunluk");//22
        dt.Columns.Add("SGKPrimHesabinaEsasUcretBildirimi");//23
        ...
        var getir = (from ogr in turna.tblOgrenci
                     ...
                     where calisti.calisacagiCalistigiYer == birimID &&
                     (((sgk.SGKCikis.Value.Month >= bordroAy && sgk.SGKCikis.Value.Year >= bordroYil)) || sgk.SGKCikis == null)
        ...
        foreach (var item in getir)
        {
            ...
            var sgkGetir = from sgkGC in turna.tblOgrenciMaas
                           ...
                           orderby sgkGC.id descending
                           select new { sgkGC.SGKGirisTarihi, sgkGC.SGKCikisTarihi };

            string sgkG = "-1";
            string sgkC = "-1";

            foreach (var itemSgk in sgkGetir.Take(1))
            {

...
public partial class SKSEkrani_PuantajCikti : System.Web.UI.Page
{
    int puantajAy;
    int puantajYıl;
    int toplamSay = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        foreach (var item in getir)
        {
           ...
           // else if (item.GSS2243 == "43")
            ...
            int Yil = Convert.ToInt16(Session["PuantajYil"]);
            var yoklamaGetir = from ogr in turna.tblOgrenci
                               ...
                               where yoklama.tarih.Value.Month == ay &&
                                   yoklama.tarih.Value.Year == Yil && ogr.ogrNo == item.ogrNo
    ...
    protected void DataList1_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            int hazirlayanKim = Convert.ToInt16(Session["hazirlayan"]);
            int onaylayanKim = Convert.ToInt16(Session["onaylayan"]);
            var hazirlayan = (from hazirlayanBul in turna.tblHazirlayanOnaylayan where hazirlayanBul.id == hazirlayanKim select new { hazirlayanBul.ad, hazirlayanBul.soyad,hazirlayanBul.unvan }).FirstOrDefault();
            var onaylayan = (from onaylayanBul in turna.tblHazirlayanOnaylayan where onaylayanBul.id == onaylayanKim select new { onaylayanBul.ad, onaylayanBul.soyad ,onaylayanBul.unvan}).FirstOrDefault();
            if (e.Item.ItemType == ListItemType.Footer)
            {
                ((Label)e.Item.FindControl("lblHazirlayan")).Text =" "+ hazirlayan.ad + " " + hazirlayan.soyad;
                ((Label)e.Item.FindControl("lblOnaylayan")).Text = " "+onaylayan.ad + " " + onaylayan.soyad;
                ((Label)e.Item.FindControl("lblHazirlayanUnvan")).Text = " "+hazirlayan.unvan;
                ((Label)e.Item.FindControl("lblOnaylayanUnvan")).Text = " "+onaylayan.unvan;
